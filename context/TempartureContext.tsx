...
const TempratureContextProvider: React.FC<React.ReactNode> = ({children}) => {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (status !== "granted") {
        ...
        const URL = `https://api.openweathermap.org/data/2.5/onecall?lat=${Longitude_Latitude["latitude"]}&lon=${Longitude_Latitude["longitude"]}&units=metric&appid=c80d19271b144cb9f3081ce5ba01715d`;
        try {
  ...
  const getStateWeatherData = async (cityVal: any) => {
    ...
    try {
      const URL = `https://api.openweathermap.org/data/2.5/weather?q=${cityVal}&units=metric&appid=c80d19271b144cb9f3081ce5ba01715d`;
      const res = await fetch(URL);
      const data = await res.json();
